    ...
    return async (input) => {
      ...
      switch (analysis.intent) {
        ...
        case 'explore':
          ...
          const thought = await pipeline.processThought(instanceId, message, {
            source: 'exploration',
            conversationId
          });
          
          return {
            response: `Interesting perspective! ${formatExplorationResponse(thought)}`,
            thoughtsCaptured: 1,
            sessionId: session.sessionId
          };
          
        default:
          // General conversation - still capture the thought
          const generalThought = await pipeline.processThought(instanceId, message, {
            source: 'conversation',
            conversationId
          });
...
function formatExplorationResponse(thought) {
  if (thought.significance >= 7) {
    return `This sparked something significant. I've captured it with high priority.`;
  }
