...
export const validationAndFixesTool = {
    ...
    async handler(args, context) {
        ...
        if (sandboxTest && result.ccmcpResult?.success) {
            ...
            try {
                sandboxResults = await context.gmcpManager.executeTask(`Test the following fixes: ${JSON.stringify(result.ccmcpResult.data)}`, undefined, // model
                true, // sandbox
...
function extractIssues(validationData) {
    ...
    if (validationData.output && typeof validationData.output === 'string') {
        ...
        const issueLines = lines.filter((line) => line.includes('error') ||
...
//# sourceMappingURL=validationAndFixes.js.map