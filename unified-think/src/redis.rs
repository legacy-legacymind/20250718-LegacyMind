...
impl RedisManager {
    ...
    pub async fn new() -> Result<Self> {
        ...
        if !redis_password.is_empty() {
            ...
            if redis_password.contains('@') || redis_password.contains(':') || redis_password.contains('/') {
                ...
                return Err(UnifiedThinkError::Configuration("Invalid Redis password format - contains URL-unsafe characters".to_string()));
            }
        }
        ...
        match result {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("unknown command") || e.to_string().contains("ERR unknown command 'BF.RESERVE'") {
                    Err(UnifiedThinkError::Configuration(
                    ...
                    Err(UnifiedThinkError::Internal(format!("Failed to create bloom filter: {}", e)))
                }
            }
        }
    }
        ...
        match result {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("unknown command") || e.to_string().contains("ERR unknown command 'BF.EXISTS'") {
                    ...
                    Err(UnifiedThinkError::Internal(format!("Failed to check bloom filter: {}", e)))
                }
            }
        }
    }
            ...
            Err(e) => {
                ...
                if e.to_string().contains("unknown command") || e.to_string().contains("ERR unknown command 'BF.ADD'") {
                    ...
                    match retry_result {
                        ...
                        Err(e) => {
                            Err(UnifiedThinkError::Internal(format!("Failed to add to bloom filter after creation: {}", e)))
                        }
                    }
                } else {
                    Err(UnifiedThinkError::Internal(format!("Failed to add to bloom filter: {}", e)))
                }
            }
        }
    }
        ...
        match result {
            ...
            Err(e) => {
                if e.to_string().contains("unknown command") || e.to_string().contains("not found") {
                    ...
                    Err(UnifiedThinkError::Internal(format!("Failed to get bloom filter info: {}", e)))
                }
            }
        }
    }
        ...
        F: Future<Output = Result<T>>,
    {
        match timeout(duration, operation).await {
            Ok(result) => result,
            Err(_) => Err(UnifiedThinkError::Timeout(duration.as_secs())),
        }
    }
    ...
    pub async fn load_scripts(&self) -> Result<()> {
        ...
        scripts.store_thought = store_script.prepare_invoke()
            .load_async(&mut *conn)
            .await
            .map_err(|e| UnifiedThinkError::Internal(format!("Failed to load store thought script: {}", e)))?;
        ...
        scripts.get_thought = get_script.prepare_invoke()
            .load_async(&mut *conn)
            .await
            .map_err(|e| UnifiedThinkError::Internal(format!("Failed to load get thought script: {}", e)))?;
        ...
        scripts.search_thoughts = search_script.prepare_invoke()
            .load_async(&mut *conn)
            .await
            .map_err(|e| UnifiedThinkError::Internal(format!("Failed to load search thoughts script: {}", e)))?;
        ...
        scripts.update_chain = update_chain_script.prepare_invoke()
            .load_async(&mut *conn)
            .await
            .map_err(|e| UnifiedThinkError::Internal(format!("Failed to load update chain script: {}", e)))?;
        ...
        scripts.get_chain_thoughts = get_chain_script.prepare_invoke()
            .load_async(&mut *conn)
            .await
            .map_err(|e| UnifiedThinkError::Internal(format!("Failed to load get chain thoughts script: {}", e)))?;
        ...
        scripts.cleanup_expired = cleanup_script.prepare_invoke()
            .load_async(&mut *conn)
            .await
            .map_err(|e| UnifiedThinkError::Internal(format!("Failed to load cleanup expired script: {}", e)))?;
        ...
        let result: String = redis::cmd("EVALSHA")
            ...
            .or_else(|e| {
                ...
                if e.to_string().contains("NOSCRIPT") {
                    ...
                    return Err(UnifiedThinkError::Internal("Script needs reloading".to_string()));
                }
                Err(UnifiedThinkError::Redis(e))
            })?;
        
        match result.as_str() {
            "OK" => Ok(true),
            "DUPLICATE" => Ok(false),
            _ => Err(UnifiedThinkError::Internal(format!("Unexpected script result: {}", result))),
        }
    }
        ...
        match result {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("unknown command") || e.to_string().contains("ERR unknown command 'VSET.CREATE'") {
                    ...
                    Err(UnifiedThinkError::Internal(format!("Failed to create vector set: {}", e)))
                }
            }
        }
    }
            ...
            Err(e) => {
                if e.to_string().contains("unknown command") {
                    ...
                    match retry_result {
                        ...
                        Err(e) => {
                            Err(UnifiedThinkError::Internal(format!("Failed to add vector after creation: {}", e)))
                        }
                    }
                } else {
                    Err(UnifiedThinkError::Internal(format!("Failed to add thought vector: {}", e)))
                }
            }
        }
    }
        ...
        match result {
            ...
            Err(e) => {
                if e.to_string().contains("unknown command") || e.to_string().contains("not found") {
                    ...
                    Err(UnifiedThinkError::Internal(format!("Failed to search similar thoughts: {}", e)))
                }
            }
        }
    }
        ...
        match result {
            ...
            Err(e) => {
                if e.to_string().contains("unknown command") || e.to_string().contains("not found") {
                    ...
                    Err(UnifiedThinkError::Internal(format!("Failed to get vector set info: {}", e)))
                }
            }
        }
    }
