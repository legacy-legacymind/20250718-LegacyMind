...
use crate::error::{UnifiedThinkError, Result};
...
impl RateLimiter {
    ...
    /// * `Err(UnifiedThinkError::RateLimit)` if rate limit exceeded
    pub async fn check_rate_limit(&self, instance_id: &str) -> Result<()> {
        ...
        if timestamps.len() >= self.max_requests {
            ...
            return Err(UnifiedThinkError::RateLimit);
        }
    ...
    async fn test_rate_limiter_blocks_over_limit() {
        ...
        assert!(
            matches!(
                ...
                Err(UnifiedThinkError::RateLimit)
            ),
